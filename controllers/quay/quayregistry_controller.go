...
func (r *QuayRegistryReconciler) hasNecessaryConfig(
	...
	for _, cmp := range quay.Spec.Components {
		...
		if cmp.Kind == v1.ComponentTLS && quay.Spec.TLS != nil && quay.Spec.TLS.SecretRef != nil {
			continue
		}
...
func (r *QuayRegistryReconciler) findQuayRegistriesForSecret(
	...
	if err := r.Client.List(ctx, &registries, &client.ListOptions{
	...
	for _, reg := range registries.Items {
		...
		if reg.Spec.TLS.SecretRef.Name != secret.GetName() {
			continue
		}
