...
// checkRoutesAvailable verifies if the config bundle contains an etry pointing to the buildman
// host. If it contains then sets it properly in the provided QuayRegistryContext
...
func (r *QuayRegistryReconciler) checkMonitoringAvailable(
	...
	if err := r.Client.Get(ctx, namespaceKey, &grafanaDashboardNamespace); err != nil {
		msg := fmt.Sprintf(
			"Unable to find the Grafana config namespace %s. "+
