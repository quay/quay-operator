...
func SetDefaults(client client.Client, quayConfiguration *resources.QuayConfiguration) bool {
	...
	if quayConfiguration.QuayEcosystem.Spec.Clair != nil && quayConfiguration.QuayEcosystem.Spec.Clair.Enabled == true {
		...
		if utils.IsZeroOfUnderlyingType(quayConfiguration.QuayEcosystem.Spec.Clair.ClairDatabaseSslMode) {
			changed = true
			quayConfiguration.QuayEcosystem.Spec.Clair.ClairDatabaseSslMode = constants.ClairDatabaseSslModeDefault
		}
