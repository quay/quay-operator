...
func (v *QuayRegistryValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	log.Info("examining object",
		...
		"name", req.Name,
		"kind", req.Kind)
	// TODO(jonathankingfc): Validate `spec.components` based on feature detection...

	// TODO(jonathankingfc): Validate `spec.configBundleSecret`, check for (un)managed components config values, check provided TLS cert/key pair, etc...

	return admission.Denied("TODO(jonathankingfc): Not implemented")
}
